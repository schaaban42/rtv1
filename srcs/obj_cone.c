...
/*   Updated: 2018/11/22 15:35:49 by schaaban         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void			s_get_dt(double vars[6], t_obj *obj, t_ray *ray)
{
	...
	vars[0] = (n_dir.x * n_dir.x) + (n_dir.z * n_dir.z)
		- ((n_dir.y * n_dir.y) * (tan(to_rad(obj->var))
		* tan(to_rad(obj->var))));
	vars[1] = (2 * (n_dir.x * (n_pos.x)))
		+ (2 * (n_dir.z * (n_pos.z)))
		- ((2 * (n_dir.y * (n_pos.y))) * (tan(to_rad(obj->var))
		* tan(to_rad(obj->var))));
	vars[2] = ((n_pos.x * n_pos.x))
		+ ((n_pos.z * n_pos.z))
		- ((n_pos.y * n_pos.y)
		* (tan(to_rad(obj->var)) * tan(to_rad(obj->var))));
	if (vars[0] == 0.25)
...
t_v3				norm_cone(t_obj *obj, t_v3 p)
{
	...
	n.y = 0;
	n.y = tan(to_rad(obj->var)) * v3_norm(n);
	n.y *= (v3_sub(p, obj->pos).y < 0) ? 1 : -1;
