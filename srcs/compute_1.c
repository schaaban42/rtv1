	...
	if (rt->ray.hit.done)
	{
		...
		if (rt->light.hit.done)
		{
			...
			if (rt->light.hit.obj->type != rt->ray.hit.obj->type)
				color = color_value(color, RT_LIGHT_SHADOW);
			else
			{
				...
				else if (rt->light.hit.obj->type == 'p')
				{
					...
					else
					{
						...
						double brill = 0.95;
						if (cosa > brill)
							color = (Uint32)ft_colorgradient(((cosa - brill) * (1.0 / (1.0 - brill))), 2, color_value(color, brill), 0xffffff) | 0xff000000;
					}
				}
				else if (rt->light.hit.obj->type == 'c')
				{
					t_v3	n;
					double	cosa;

					n = norm_cylinder(rt->light.hit.obj, rt->ray.hit.pos);
					cosa = v3_ang(n, v3_sub(rt->light.pos, rt->light.hit.pos));
					color = (Uint32)ft_colorgradient(cosa, 2, 0, rt->ray.hit.obj->color) | 0xff000000;
					double brill = 0.90;
					if (cosa > brill)
				...
				else if (rt->light.hit.obj->type == 'o')
				{
					t_v3	n;
					double	cosa;

					n = norm_cone(rt->light.hit.obj, rt->ray.hit.pos);
					cosa = v3_ang(n, v3_sub(rt->light.pos, rt->light.hit.pos));
					color = (Uint32)ft_colorgradient(cosa, 2, 0, rt->ray.hit.obj->color) | 0xff000000;
					double brill = 0.90;
					if (cosa > brill)
